        ...
        if uploaded_file is not None:
            ...
            if analyze_btn:
                with st.spinner('جاري التحليل السريع باستخدام TFLite...'):
                    ...
                    # -----------------------------------------------------
                    # 3. المنطق المطور للتمييز بين الحالات (Thresholds)
                    # -----------------------------------------------------
                    
                    # حالة 1: احتمالية عالية جداً تشير إلى ورم خبيث
                    if prediction_prob > 0.80:
                        st.error("🚨 **النتيجة: مؤشرات قوية لورم خبيث (Malignant)**")
                        st.write("تم رصد أنماط بصرية تتطابق مع خصائص الأورام الجلدية السرطانية.")
                        st.warning("⚠️ **تنبيه:** يرجى التوجه لطبيب أورام فوراً.")

                    # حالة 2: احتمالية متوسطة تشير إلى آفة تستدعي المتابعة
                    elif 0.50 <= prediction_prob <= 0.80:
                        st.warning("🔍 **النتيجة: آفة جلدية تستدعي الفحص (Suspicious)**")
                        st.write("الأنماط غير واضحة تماماً، يوصى بالفحص السريري للتأكد.")

                    # حالة 3: احتمالية منخفضة جداً تشير إلى جلد سليم
                    else:
                        st.balloons()
                        st.success("✅ **النتيجة: سليم (Normal Skin/Benign)**")
                        st.write("الخصائص البصرية تظهر أنسجة طبيعية أو شامات حميدة.")

                    st.markdown("---")
                    # نسبة الثقة محذوفة بناءً على طلبك
