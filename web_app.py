        ...
        if uploaded_file is not None:
            ...
            if analyze_btn:
                with st.spinner('جاري التحليل السريع باستخدام TFLite...'):
                    ...
                    # --- تعديل لاستقبال احتمالات الفئات الثلاث ---
                    output_data = interpreter.get_tensor(output_details[0]['index'])[0]
                    
                    # --- ضبط الترتيب بناءً على تدريبك ---
                    # الترتيب الافتراضي هنا: [سليم، حميد، خبيث]
                    # يرجى عكس الترتيب إذا كان النموذج يخطئ في التفريق
                    ...
                    # -----------------------------------------------------
                    # 3. المنطق المطور لزيادة الحساسية (Thresholds)
                    # -----------------------------------------------------
                    
                    # 1. حالة الخبيث: إذا كانت النسبة > 30% اعتبره خبيث (حساس جداً)
                    if prob_khabit > 0.30:
                        st.error(f"🚨 النتيجة: ورم خبيث (Malignant)")
                        st.write("⚠️ تنبيه: تم رصد مؤشرات لآفة تستدعي الفحص الطبي الفوري.")
                    
                    # 2. حالة الحميد: إذا لم يكن خبيثاً وكان الحميد أعلى من السليم
