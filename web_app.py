...
# --- 1. إعدادات الصفحة (ضبط الحجم ليكون مناسباً) ---
st.set_page_config(
...
if file:
    ...
    if st.button(t['analyze']):
        with st.spinner("..."):
            ...
            preds = model.predict(processed_img if 'processed_img' in locals() else img_array)[0]
            idx = np.argmax(preds)
            
            # التصنيف المطلوب (خبيث، سليم، عام) فقط بدون اسم مرض أو نسبة
            if idx in [0, 1, 4]: # مؤشرات الخباثة في HAM10000
                res_msg, color = t['res_m'], "#cf1322"
            elif idx in [2, 3, 5, 6]: # مؤشرات السليمة
