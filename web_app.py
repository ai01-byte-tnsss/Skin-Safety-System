            ...
            if analyze_btn:
                with st.spinner('جاري التحليل السريع باستخدام TFLite...'):
                    ...
                    if len(output_data) == 3:
                        ...
                        # --- تطبيق الحدود العالمية لحساسية طبية عالية ---
                        # 0.30 للخبيث لتقليل تفويت الحالات القاتلة
                        if prob_khabit > 0.30:
                            st.error(f"🚨 النتيجة: ورم خبيث (Malignant)")
                            st.write("⚠️ تنبيه: تم رصد مؤشرات لآفة تستدعي الفحص الطبي الفوري.")
                        
                        # 0.50 كحد أدنى للحميد
                        ...
                        if prediction > 0.30: # اعتماد نفس الحد للخبيث
                        ...
                        st.error("⚠️ خطأ في تفسير مخرجات النموذج.")
