...
LANG_DATA = {
    ...
    "हिन्दी": {"dir": "ltr", "title": "🛡️ त्वचा सुरक्षा एआई", "note": "⚠️ सर्वोत्तम परिणामों के लिए, अच्छी प्राकृतिक रोशनी का उपयोग करें।", "upload": "📥 अपलोड", "camera": "📸 कैमरा", "analyze": "🚀 विश्लेषण", "guide": "📚 गाइड", "malig": "🔴 घातक", "benign": "🟢 सौम्य", "res_m": "🚨 घातक संदेह", "res_b": "🔍 सौम्य", "res_g": "🩺 सामान्य", "advice": "डॉक्टर से मिलें।", "share_res": "📤 साझा करें", "share_app": "🌐 वेबसाइट साझा करें"},
    "日本語": {"dir": "ltr", "title": "🛡️ 皮膚安全AI", "note": "⚠️ 最良の結果を得るには、自然光を使用してください。", "upload": "📥 アップロード", "camera": "📸 カメラ", "analyze": "🚀 解析", "guide": "📚 ガイド", "malig": "🔴 悪性", "benign": "🟢 良性", "res_m": "🚨 悪性の疑い", "res_b": "🔍 良性", "res_g": "🩺 一般", "advice": "医師に相談。", "share_res": "📤 共有する", "share_app": "🌐 サイトを共有"},
...
<style>
    ...
    .note-box {{ background: #fffbe6; border: 1px solid #ffe58f; padding: 15px; border-radius: 8px; margin-bottom: 20px; color: #856404; font-size: 0.95em; }}
    .share-container {{ display: flex; gap: 10px; justify-content: center; margin-top: 20px; flex-wrap: wrap; }}
    .share-btn {{ padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: bold; color: white !important; cursor: pointer; border: none; font-size: 0.9em; }}
</style>
""", unsafe_allow_html=True)

# --- 4. المحرك ومعالجة الصور التلقائية (الدقة والـ Float) ---
@st.cache_resource
...
def prepare_image_precision(image, interpreter):
    ...
    target_dtype = input_details[0]['dtype'] # استخراج النوع تلقائياً (float16/float32)
    
    img_rgb = image.convert("RGB")
    # استخدام LANCZOS لأعلى دقة في تغيير الحجم
        ...
        if interpreter:
            with st.spinner("Analyzing..."):
                try:
                    ...
                    # المجموعات (تأكد من مطابقتها لنموذجك)
                    if idx in [1, 4, 17]:
                    ...
                    # --- أزرار مشاركة النتيجة للمريض ---
                    share_text = urllib.parse.quote(f"*{t['title']}*\nالنتيجة: {res_msg}\n{t['advice']}")
                    st.markdown(f"""
                    <div class="share-container">
                        <p style="width:100%; text-align:center;">{t['share_res']}</p>
                        <a href="https://api.whatsapp.com/send?text={share_text}" target="_blank" class="share-btn" style="background:#25D366;">WhatsApp</a>
                        <a href="https://t.me/share/url?url=System&text={share_text}" target="_blank" class="share-btn" style="background:#0088cc;">Telegram</a>
                    </div>
                    """, unsafe_allow_html=True)
...
# --- 7. زر مشاركة الموقع في نهاية الصفحة ---
st.markdown("---")
app_link = "https://your-project-link.streamlit.app" # رابط موقعك
st.markdown(f"""
